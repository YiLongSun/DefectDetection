...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument("--gt", type=Path, required=False,
...
def main():
    ...
    for NG_OK in os.listdir(input_folder):
        ...
        for img in os.listdir(input_folder/NG_OK):

            args.input = input_folder/NG_OK/img
            args.output = ouput_folder/"{}_{}".format(NG_OK, img)

            predictions = infer(args=args)

            if NG_OK == "good":
                gt = np.zeros((predictions.pred_mask.shape))
            else:
                gt_img_name = img[:-4] + "_mask.png"
                gt = np.array(Image.open(gt_folder/NG_OK/gt_img_name))
            ...
            output_path = ouput_folder / "{}_{}".format(NG_OK, img)
